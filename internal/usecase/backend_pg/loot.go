...
func (pg *PG) CreateLoot(ctx context.Context, loot entity.Loot) (entity.Loot, error) {
	...
	rows, err := pg.Pool.Query(context.Background(), sql, loot.Name, loot.Raid.ID, loot.PlayerID)
	if err != nil {
	...
	sql, args, errInsert := pg.Builder.Insert("loots").Columns("name", "raid_id", "player_id").Values(loot.Name, loot.Raid.ID, loot.PlayerID).ToSql()
	if errInsert != nil {
...
func (pg *PG) UpdateLoot(ctx context.Context, loot entity.Loot) error {
	...
	sql, args, err := pg.Builder.Update("loots").Set("name", loot.Name).Set("raid_id", loot.Raid.ID).Set("player_id", loot.PlayerID).Where("id = $1").ToSql()
	if err != nil {
...
func (pg *PG) DeleteLoot(ctx context.Context, lootID int) error {
	...
	if errInsert != nil {
		fmt.Errorf("database - DeleteLoot - r.Builder: %w", errInsert)
	}
	_, err := pg.Pool.Exec(context.Background(), sql, lootID)
	if err != nil {
		fmt.Errorf("database - DeleteLoot - r.Pool.Exec: %w", err)
	}
	return nil
}
