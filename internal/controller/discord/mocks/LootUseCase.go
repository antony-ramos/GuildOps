...
// CreateLoot provides a mock function with given fields: ctx, lootName, raidDate, playerName
func (_m *LootUseCase) CreateLoot(ctx context.Context, lootName string, raidDate time.Time, playerName string) error {
	ret := _m.Called(ctx, lootName, raidDate, playerName)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, string, time.Time, string) error); ok {
		r0 = rf(ctx, lootName, raidDate, playerName)
	} else {
		r0 = ret.Error(0)
	}
