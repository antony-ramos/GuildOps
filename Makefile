...
artifact: ## Compile app from sources (linux)
	@CGO_ENABLED=0 GOARCH=amd64 GOOS=linux go build -a -installsuffix cgo -ldflags '-extldflags "-static"' -o gobin ./cmd/guildops

artifact.osx: ## Compile app from sources (osx)
	@CGO_ENABLED=0 GOARCH=amd64 GOOS=darwin go build -a -installsuffix cgo -ldflags '-extldflags "-static"' -o gobin ./cmd/guildops

image-ci: ## Build an image for CI Test Helm
	docker build . --tag "ghcr.io/antony-ramos/guildops:ci"
